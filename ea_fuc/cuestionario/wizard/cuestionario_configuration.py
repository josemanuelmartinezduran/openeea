...
class cuestionario_configuration(osv.osv_memory):
    _name='cuestionario.configuration'
    _description = 'Directory Configuration'
    _inherit = 'res.config'

    def execute(self, cr, uid, ids, context=None):
        dir_pool = self.pool.get('cuestionario.directory')
        data_pool = self.pool.get('ir.model.data')
        model_pool = self.pool.get('ir.model')
        content_pool = self.pool.get('cuestionario.directory.content')
        if self.pool.get('sale.order'):
            # Sale order
            dir_data_id = data_pool._get_id(cr, uid, 'cuestionario', 'dir_sale_order_all')
            if dir_data_id:
            ...
            # Qutation
            dir_data_id = data_pool._get_id(cr, uid, 'cuestionario', 'dir_sale_order_quote')
            if dir_data_id:
        ...
        if self.pool.get('product.product'):
            # Product
            dir_data_id = data_pool._get_id(cr, uid, 'cuestionario', 'dir_product')
            if dir_data_id:
        ...
        if self.pool.get('account.analytic.account'):
            # Project
            dir_data_id = data_pool._get_id(cr, uid, 'cuestionario', 'dir_project')
            if dir_data_id:
