...
class cuestionario_file(osv.osv):
    ...
    _columns = {
        ...
        # Fields of cuestionario:
        'user_id': fields.many2one('res.users', 'Owner', select=1),
        'parent_id': fields.many2one('cuestionario.directory', 'Directory', select=1, change_default=True),
    ...
    def check(self, cr, uid, ids, mode, context=None, values=None):
        ...
        super(cuestionario_file, self).check(cr, uid, ids, mode, context=context, values=values)
        if ids:
            self.pool.get('ir.model.access').check(cr, uid, 'cuestionario.directory', mode)
            ...
            self.pool.get('cuestionario.directory').check_access_rule(cr, uid, [parent_id for (parent_id,) in cr.fetchall()], mode, context=context)

    def search(self, cr, uid, args, offset=0, limit=None, order=None, context=None, count=False):
        # Grab ids, bypassing 'count'
        ids = super(cuestionario_file, self).search(cr, uid, args, offset=offset, limit=limit, order=order, context=context, count=False)
        if not ids:
            return 0 if count else []

        # Filter out cuestionarios that are in directories that the user is not allowed to read.
        ...
        # filter parents
        visible_parent_ids = self.pool.get('cuestionario.directory').search(cr, uid, [('id', 'in', list(parent_ids))])
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        return super(cuestionario_file, self).copy(cr, uid, id, default, context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        return super(cuestionario_file, self).create(cr, uid, vals, context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if vals.get('datas', False):
            vals['file_type'], vals['index_content'] = self._index(cr, uid, vals['datas'].decode('base64'), vals.get('datas_fname', False), None)
        return super(cuestionario_file, self).write(cr, uid, ids, vals, context)
...
class cuestionario_directory(osv.osv):
    _name = 'cuestionario.directory'
    ...
    _columns = {
        ...
        'group_ids': fields.many2many('res.groups', 'cuestionario_directory_group_rel', 'item_id', 'group_id', 'Groups'),
        'parent_id': fields.many2one('cuestionario.directory', 'Parent Directory', select=1, change_default=True),
        'child_ids': fields.one2many('cuestionario.directory', 'parent_id', 'Children'),
        'file_ids': fields.one2many('ir.attachment', 'parent_id', 'Files'),
        'content_ids': fields.one2many('cuestionario.directory.content', 'directory_id', 'Virtual Files'),
        ...
        'dctx_ids': fields.one2many('cuestionario.directory.dctx', 'dir_id', 'Context fields'),
        'company_id': fields.many2one('res.company', 'Company', change_default=True),
    }

    _defaults = {
        'company_id': lambda s,cr,uid,c: s.pool.get('res.company')._company_default_get(cr, uid, 'cuestionario.directory', context=c),
        'user_id': lambda self,cr,uid,ctx: uid,
        'domain': '[]',
        'type': 'directory',
        'ressource_id': 0,
    ...
    def _check_recursion(self, cr, uid, ids, context=None):
        level = 100
        while len(ids):
            cr.execute('select distinct parent_id from cuestionario_directory where id in ('+','.join(map(str,ids))+')')
    ...
    def _prepare_context(self, cr, uid, nctx, context=None):
        ...
        ORM instance of cuestionario.directory).
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        default.update(name=_("%s (copy)") % (name))
        return super(cuestionario_directory,self).copy(cr, uid, id, default, context=context)
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        return super(cuestionario_directory,self).write(cr, uid, ids, vals, context=context)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        return super(cuestionario_directory,self).create(cr, uid, vals, context)

class cuestionario_directory_dctx(osv.osv):
    ...
    """
    _name = 'cuestionario.directory.dctx'
    _description = 'Directory Dynamic Context'
    _columns = {
        'dir_id': fields.many2one('cuestionario.directory', 'Directory', required=True, ondelete="cascade"),
...
class cuestionario_directory_content_type(osv.osv):
    _name = 'cuestionario.directory.content.type'
...
class cuestionario_directory_content(osv.osv):
    _name = 'cuestionario.directory.content'
    _description = 'Directory Content'
    _order = "sequence"

    def _extension_get(self, cr, uid, context=None):
        cr.execute('select code,name from cuestionario_directory_content_type where active')
        res = cr.fetchall()
        return res
    ...
    _columns = {
        ...
        'directory_id': fields.many2one('cuestionario.directory', 'Directory'),
    }
    _defaults = {
...
class cuestionario_storage(osv.osv):
    """ The primary object for data storage. Deprecated.  """
    _name = 'cuestionario.storage'
...
class node_context(object):
    ...
    def __init__(self, cr, uid, context=None):
        ...
        if context is None:
            context = {}
        context['uid'] = uid
        self._dirobj = pooler.get_pool(cr.dbname).get('cuestionario.directory')
    ...
    def get_dir_node(self, cr, dbro):
        """Create (or locate) a node for a directory
            @param dbro a browse object of cuestionario.directory
        """
...
class node_class(object):
    """ this is a superclass for our inodes
        It is an API for all code that wants to access the cuestionario files.
...
class node_dir(node_database):
    ...
    def __eq__(self, other):
        ...
        if not self.context == other.context:
            return False
        # Two directory nodes, for the same cuestionario.directory, may have a
    ...
    def _file_get(self, cr, nodename=False):
        ...
        cntobj = self.context._dirobj.pool.get('cuestionario.directory.content')
        uid = self.context.uid
        ctx = self.context.context.copy()
        ctx.update(self.dctx)
    ...
    def rmcol(self, cr):
        ...
        if directory._table_name=='cuestionario.directory':
            if self.children(cr):
    ...
    def move_to(self, cr, ndir_node, new_name=False, fil_obj=None, ndir_obj=None, in_write=False):
        ...
        only used for static, simple directories.
            Note /may/ be called with ndir_node = None, to rename the cuestionario root.
        """
...
class node_res_dir(node_class):
    ...
    def __eq__(self, other):
        ...
        if not self.context == other.context:
            return False
        # Two nodes, for the same cuestionario.directory, may have a
        # different context! (dynamic folders)
        if self.dctx != other.dctx:
            return False
        return self.dir_id == other.dir_id
...
class node_res_obj(node_class):
    ...
    def get_dav_props_DEPR(self, cr):
        # Deprecated! (but cuestionario_ics must be cleaned, first)
        res = {}
        cntobj = self.context._dirobj.pool.get('cuestionario.directory.content')
        uid = self.context.uid
        ctx = self.context.context.copy()
...
class node_file(node_class):
    ...
    def open_data(self, cr, mode):
        ...
        stobj = self.context._dirobj.pool.get('cuestionario.storage')
    ...
    def rm(self, cr):
        ...
        if not self.check_perms(8):
            raise IOError(errno.EPERM, "Permission denied.")
        cuestionario_obj = self.context._dirobj.pool.get('ir.attachment')
        if self.type in ('collection','database'):
            return False
        cuestionario = cuestionario_obj.browse(cr, uid, self.file_id, context=self.context.context)
        res = False
        if cuestionario and cuestionario._table_name == 'ir.attachment':
            res = cuestionario_obj.unlink(cr, uid, [cuestionario.id])
        return res
    ...
    def fix_ppath(self, cr, fbro):
        ...
        dirpath = []
        if fbro.parent_id:
            dirobj = self.context._dirobj.pool.get('cuestionario.directory')
...
class node_content(node_class):
    ...
    def fill_fields(self, cr, dctx=None):
        ...
        cr.execute('SELECT DISTINCT mimetype FROM cuestionario_directory_content_type WHERE active AND code = %s;',
                (self.extension,))
        res = cr.fetchall()
        if res and res[0][0]:
...
class nodefd_content(StringIO, node_descriptor):
    ...
    def __init__(self, parent, cr, mode, ctx):
        ...
        if mode in ('r', 'r+'):
            cntobj = parent.context._dirobj.pool.get('cuestionario.directory.content')
    ...
    def close(self):
        ...
        try:
            if self.mode in ('w', 'w+', 'r+'):
                data = self.getvalue()
                cntobj = par.context._dirobj.pool.get('cuestionario.directory.content')
